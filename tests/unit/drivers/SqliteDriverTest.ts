...
export default class SqliteDriverTest {
  ...
  public async shouldBeAbleToCloneTheDriverInstance({ assert }: Context) {
    ...
    assert.instanceOf(result, SqliteDriver)
  }
  ...
  public async shouldBeAbleToGetTheDatabasesOfDriver({ assert }: Context) {
    ...
    assert.deepEqual(databases, ['main'])
  }
  ...
  public async shouldBeAbleToGetTheCurrentDatabaseNameThatIsBeingUsed({ assert }: Context) {
    ...
    assert.isUndefined(database)
  }

  @Test()
  public async shouldBeAbleToValidateThatDatabaseExists({ assert }: Context) {
    const exists = await this.driver.hasDatabase('main')
  ...
  public async shouldBeAbleToAddAHavingNotBetweenClauseToTheQueryUsingDriver({ assert }: Context) {
    ...
    const data = await this.driver.table('users').groupBy('id', 'name').havingNotBetween('id', ['1', '3']).findMany()
  ...
  public async shouldBeAbleToAddAOrHavingNotBetweenClauseToTheQueryUsingDriver({ assert }: Context) {
    ...
    const data = await this.driver.table('users').groupBy('id', 'name').orHavingNotBetween('id', ['1', '3']).findMany()
  ...
  public async shouldBeAbleToAddAWhereFullTextClauseWithMultipleColumnsToTheQueryUsingDriver({ assert }: Context) {
    ...
    await this.driver.table('users').createMany([
      ...
      { id: '3', name: 'Charlie Munger', email: 'charlie@athenna.io' }
    ])
  ...
  public async shouldBeAbleToAddAWhereNotBetweenClauseToTheQueryUsingDriver({ assert }: Context) {
    ...
    const data = await this.driver.table('users').whereNotBetween('id', ['1', '3']).findMany()
  ...
  public async shouldBeAbleToAddAOrWhereNotBetweenClauseToTheQueryUsingDriver({ assert }: Context) {
    ...
    const data = await this.driver.table('users').orWhereNotBetween('id', ['1', '3']).findMany()
  ...
  public async shouldCompileScalarWhereJsonWithJsonExtract({ assert }: Context) {
    ...
    assert.include(text, "json_extract(`metadata`, '$.key') = 'value'")
    assert.include(number, "json_extract(`metadata`, '$.key') > 1")
    assert.include(nested, "json_extract(`metadata`, '$.a.b') = 'x'")
    assert.include(wildcard, 'json_each')
  }
  ...
  public async shouldBeAbleToOrderByAJsonPathUsingDriver({ assert }: Context) {
    await this.driver.table('events').createMany([
      { id: '1', metadata: { title: 'b' } },
      { id: '2', metadata: { title: 'a' } },
      { id: '3', metadata: null }
    ])
